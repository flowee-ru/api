...
func GetInfo(w http.ResponseWriter, r *http.Request, db *mongo.Database) {
	...
	err := db.Collection("accounts").FindOne(context.TODO(), bson.D{primitive.E{Key: "_id", Value: id}, primitive.E{Key: "isActive", Value: true}}).Decode(&account)
	if err == mongo.ErrNoDocuments {
